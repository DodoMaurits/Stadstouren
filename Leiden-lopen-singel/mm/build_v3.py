...
page_order = [
    'Leiden-Lsingel-mm.html',  # 0 - Intro (ZONDER startTimerButton in de pagina zelf)
...
for filename in html_files:
    ...
    # Haal verdachten-grid WEG
    verdachten_grid = body.find('div', {'id': 'verdachtenGrid', 'class': 'verdachten-grid'})
    if verdachten_grid:
        verdachten_grid.decompose()
    
    # Haal nav-buttons WEG uit ALLE pagina's (we hebben globale)
    nav_buttons = body.find('div', class_='nav-buttons')
    if nav_buttons:
        nav_buttons.decompose()
    
    # Speciale behandeling voor intro-pagina: verwijder startTimerButton
    if filename == 'Leiden-Lsingel-mm.html':
        start_button = body.find('a', {'id': 'startTimerButton'})
        if start_button:
            start_button.decompose()
...
<head>
    ...
    <style>
        ...
        /* Verberg de verdachten-grid en nav-buttons in de pagina's zelf */
...
# Voeg alle pagina's toe (ZONDER verdachten-grid en ZONDER nav-buttons)
...
html_output += '''            ];
            ...
            function navigateTo(pageIndex) {
                if (pageIndex < 0 || pageIndex >= pages.length) {
                    console.error('Invalid page index:', pageIndex, 'Max:', pages.length);
                    return;
                }
            ...
            function updateGlobalNavButtons() {
                ...
                if (navButtons) {
                    ...
                    globalNavButtons.querySelectorAll('a').forEach(link => {
                        ...
                        if (link.id === 'startTimerButton') {
                            link.addEventListener('click', (e) => {
                                ...
                                console.log('Starting timer, navigating to page index 1');
                                navigateTo(1); // FIRST QUESTION
                            });
                        }
                    ...
                    // For intro page (page 0), show start button
                    if (currentPageIndex === 0) {
                        globalNavButtons.innerHTML = '<a href="Leiden-L1-singel-mm-1vraag.html" class="nav-button next" id="startTimerButton">Start de tijd</a>';
                        const startBtn = globalNavButtons.querySelector('#startTimerButton');
                        if (startBtn) {
...
print("v3: Één startTimerButton in globale nav-buttons, navigatie naar stap 1")
